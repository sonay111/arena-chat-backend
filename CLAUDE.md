...
4. **Phase 3 — Conversation lifecycle & identity.** ✅ Done: conversation creation on join, message history sent on join, reconnection replay, and identity verification — the `join` handler (`src/server.ts`) now calls the CRM API (`src/crm/`) to look up the player id before trusting it, rejecting the connection with a clear reason if the lookup fails or finds no match. Real CRM calls fail right now with `CrmIpNotAllowedError` until our server's IP is allowlisted (expected, logged distinctly) — until then, `SKIP_IDENTITY_CHECK=true` in `.env` bypasses the lookup for local testing only, off by default, and loudly logged (startup banner + per-join warning) whenever it's on so it can't be left on by accident.
5. **Phase 4 — Customer widget.** ← **current focus now that Phase 3 is done.** The widget = **look + behavior**, not just a design. To build it we need (a) a rough look/design direction (Arena365 branding — colors, logo, feel; a reference image or sketch helps but isn't required, and it can start plain and improve later), and (b) a clear behavior spec: bubble when closed, panel when open, start a chat, send/receive messages, show typing, reconnect on network drop. Behavior matters more than pixels for v0.1. During Phases 2–3 a throwaway HTML test client stands in for the widget; the real widget replaces it here. Also: coordinate with the tech team on exactly how the plain player id is passed into the widget (their side embeds it).
